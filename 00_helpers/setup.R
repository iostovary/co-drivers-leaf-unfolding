...
species_color_scale_en <- function() { #old cross check and remove
  scale_color_manual(
    values = c("Beech" = "#6C8ED4", "Larch" = "#99AA22", "Spruce" = "#644001", "Linden" = "#FFC83D"),
    na.translate = FALSE)
}

species_color_scale <- function(lang = "short") {
  species_names <- list(
    latin = c("FS" = "Fagus sylvatica", "LD" = "Larix decidua", "TC" = "Tilia cordata"),
    english = c("FS" = "Beech", "LD" = "Larch", "TC" = "Linden"),
    german = c("FS" = "Buche", "LD" = "Lärche", "TC" = "Linde"),
    short = c("FS" = "FS", "LD" = "LD", "TC" = "TC")  # Just return short codes
  )
  
  colors <- c("FS" = "#6C8ED4", "LD" = "#99AA22", "TC" = "#FFC83D")
  
  list(
    scale_color_manual(values = colors, labels = species_names[[lang]], na.translate = FALSE),
    scale_fill_manual(values = colors, labels = species_names[[lang]], na.translate = FALSE)
  )
}
