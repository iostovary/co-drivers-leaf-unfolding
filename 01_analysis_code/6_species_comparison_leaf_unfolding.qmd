...
lib_results <- c(
  ...
  "nlme",
  "cowplot",
  "ggtext", # html formatting of superscript
  "ggh4x" # nested facets
  )
...
p_table <- coeff_table %>% 
  mutate(SPECIES_SHORT = SPECIES,
         ...
         PARAMETER = label,
         model_name = case_when(model_name %like% "parsimonious.*a" ~ "parsimonious model 1",
                          model_name %like% "parsimonious.*b" ~ "parsimonious model 2",
                          model_name %like% "parsimonious.*c" ~ "parsimonious model 3",
                          TRUE ~ model_name)) %>% 
  arrange(SPECIES,
          str_detect(term, "sd_"),
...
selection_parameters <- list.files(path = paste0(path_output_data),
  ...
  mutate(type = case_when(model_name %like% "parsimonious.*a" ~ "parsimonious model 1",
                          model_name %like% "parsimonious.*b" ~ "parsimonious model 2",
                          model_name %like% "parsimonious.*c" ~ "parsimonious model 3",
                          TRUE ~ type))
```
...
weights <- list.files(path = paste0(path_output_data),
  ...
  category = case_when(PARAMETER_ABBR %like% "CWB" ~ "Climate / Soil",
                              ...
                              category %like% "Climate" ~ "Climate / Soil",
                              TRUE ~ category),
  ...
  WEIGHT = case_when(SPECIES_SHORT == "TC" & PARAMETER == "poly(Stand height):poly(Tree height)" ~ 0.002,
                     TRUE ~ WEIGHT)
  )
...
ggplot(weights, aes(x = SPECIES, y = PARAMETER, fill = WEIGHT)) +
  ...
  geom_text(aes(label = WEIGHT), size = 3, vjust = 0, colour = "#4F4F4F") +
  xlab("") +
  ylab("") +
...
model_order <- c("best model", "parsimonious model 1", "parsimonious model 2", "parsimonious model 3", "refined model", "traditional model")
```
...
tbl <- p_table_parsimonious %>%
  mutate(model_name = case_when(model_name %like% "parsimonious.*a" ~ "parsimonious model 1",
                                model_name %like% "parsimonious.*b" ~ "parsimonious model 2",
                                model_name %like% "parsimonious.*c" ~ "parsimonious model 3",
                                TRUE ~ model_name)) %>%
...
effects_fun_num <- function(data, label, type = "line"){
  data <- data %>%
    mutate(SPECIES = recode(SPECIES_SHORT, !!!rename_species_en),
           Label = label_fun_sim(as.character(Term), col = "short_with_units")) %>% 
      filter(!is.na(x))
...
x_positions <- rep(seq(0.083, 0.78, length.out = n_cols), times = n_rows)
y_positions <- rep((seq(1.01, 0.35, length.out = n_rows)), each = n_cols)
...
```{r fig-effects-best-all-thematic, fig.width=10, fig.height=7, fig.cap="Species-specific environmental drivers beyond temperature shape the timing of leaf unfolding. Panels show the estimated effect of predictors present in species-specific best models, adjusted for the influence of other included covariates at their mean values. Shaded areas and error bars represent 95% confidence intervals."}
rotated_label <- function(label_text) {
  ggdraw() + 
    draw_line(
      x = c(-0.2, -0.2),  # shift line (to the right with negative values)
      y = c(0.05, 0.95),
      size = 1.2, color = "grey") +
    draw_label(label_text,
      x = 0.35,  # shift topic label
      angle = 90, hjust = 0.5, vjust = 0.5, size = 13)
    #theme(plot.margin = margin(t = 0, r = 0, b = 0, l = 0)) # pulls label closer
}
...
annotated_rows <- map2(aligned_plot_rows, group_titles,
  ~ plot_grid(
      ...
      ncol = 2,
      rel_widths = c(0.025, 0.95), # control distance between group label and line
      align = "v",
      axis = "tb"))
...
final_plot <- plot_grid(
  ...
  ncol = 1) # uniform row heights

final_plot_labeled <- ggdraw() +
  draw_plot(final_plot,
            x = 0.035, y = 0, # position of first panel starting with line
            width = 0.95, # panel width
            height = 1) +  
  draw_label("Day of year",
    x = 0.01, y = 0.5,
    angle = 90,
    fontface = "bold",
    size = 16) +
  draw_plot_label(
    label = panel_letters,  # reverse order of panel letters
    x = x_positions,
    y = y_positions,
    size = 14,
    fontface = "bold")
...
ggplot(prediction, aes(x = x, y = predicted)) +
  ...
  labs(x = lab_string, y = "", colour = "Group") +
  ...
  theme(axis.text.y = element_text(size = 10, vjust = 0.5, hjust = 0.5),
        axis.title.y = element_text(size = 11, vjust = 0.5, hjust = 0.5, angle=90),
        legend.position = "bottom",
        plot.margin = margin(t = 0, b = 4, r = 6, l = 0))
}
```

```{r fig-effects-warming-chilling-tc1, fig.width = 4, fig.height = 4, fig.cap="Effect of the interaction between GDD and CDD at low, medium and high CDD in the best model of linden. Predictions are adjusted to mean covariate values, with 95% confidence intervals (grey)."}
interaction_effect_fun("best_con_1", "TC", c("GDD_conservative [all]", "CDD_conservative"), "GDD (° C · days)", c("low CDD", "medium CDD", "high CDD"))
```
...
interaction_effect_fun("best_con_1", "PA", c("DROUGHT [all]", "AWC"), "CWB (mm)", c("low AWC", "medium AWC", "high AWC"))
```
